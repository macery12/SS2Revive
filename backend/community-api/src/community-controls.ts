...
function creatorIds(text: string): string[] {
  try {
...
async function ownershipRow(env: Env, mapId: string): Promise<OwnershipRow | null> {
  return env.DB.prepare(
    `SELECT m.owner_steam_id64, m.status, m.current_revision, v.creator_ids_json
       FROM maps m
       JOIN map_versions v ON v.map_id = m.id AND v.revision = m.current_revision
      WHERE m.id = ?`,
...
export async function archiveOwnMap(
  ...
  const row = await requireOwner(env, mapId, principal.steamId64);
  if (row.status === "archived") return emptyResponse(204, requestId);
  const results = await env.DB.batch([
    env.DB.prepare(
      `UPDATE maps SET status = 'archived', archived_at_ms = ?, unpublished_at_ms = COALESCE(unpublished_at_ms, ?)
        WHERE id = ? AND owner_steam_id64 = ? AND status != 'archived'`,
    ).bind(nowMs, nowMs, mapId, principal.steamId64),
    env.DB.prepare(
      `UPDATE map_versions SET status = 'unpublished' WHERE map_id = ? AND revision = ?`,
    ).bind(mapId, row.current_revision),
  ]);
...
async function moderationAction(
  ...
  const results = await env.DB.batch([
    ...
    ).bind(nowMs, principal.steamId64, `${action}: ${body.reason.trim()}`.slice(0, 512), mapId),
  ]);
  if ((results[0]?.meta.changes ?? 0) !== 1) {
